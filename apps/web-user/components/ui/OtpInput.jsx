        ...
        <div className="flex justify-between gap-2 sm:gap-4" onPaste={handlePaste}>
            {otp.map((digit, i) => (
                <input
                    ...
                    className={clsx(
                        "w-full aspect-square text-center bg-black/[0.03] dark:bg-white/[0.03] border rounded-2xl text-xl md:text-2xl font-black transition-all focus:outline-none focus:ring-2 focus:ring-orange/50",
                        digit ? "border-orange/50 text-black dark:text-white" : "border-black/10 dark:border-white/10 text-black/20 dark:text-white/20",
                        disabled && "opacity-50"
                    )}
                />
            ))}
